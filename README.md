...
- **🚀 灵活部署**：
  - **完整版 (Full)**：内置 .NET Runtime，开箱即用，无需安装依赖，体积约 60 MB。
  - **轻量版 (Lite)**：需预先安装 .NET 8 Runtime，体积极小，适合已有环境的用户。
...
2. 下载最新版本的 `MaoJi_Full.zip` (完整绿色版) 或 `MaoJi_Lite.zip` (轻量绿色版)。
3. 也可选择对应的安装包 `MaoJi_Full_Setup.exe` 或 `MaoJi_Lite_Setup.exe`。
4. 解压或安装后即可运行（Full 版已内置环境；Lite 版需安装 .NET 8）。
...
### 2025-12-26
1. **优化文本编辑**：解决在大文档编辑时出现的卡顿现象，提升编辑流畅度。
2. **降低响应延迟**：优化关闭流程，确保在用户点击关闭按钮后迅速响应。

### 2025-12-23
1. **优化主窗口 UI**：调整界面细节，视觉效果更佳。
2. **查找功能增强**：查找匹配项添加高亮底色，定位更清晰。
3. **构建系统升级**：新增 Full (完整版) 和 Lite (轻量版) 两种发布模式，满足不同需求。
